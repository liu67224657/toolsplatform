$(document).ready(function () {
    ...
    window.passport = 'http://passport' + host + '/';
...
function initPostMask() {
    if (window.uno == null || window.uno.length == 0 || window.uid == null || window.uid.length == 0 || window.token == null || window.token.length == 0 || window.timestamp == null || window.timestamp.length == 0) {
        $('#textarea_body').after('<div class="wrapper_unlogin" style="text-align:center; margin:-65px 0 65px">您需要<a id="login_link" href="' + window.passport + 'auth/loginpage?of=lh">登录</a>后才能评论</div>');
