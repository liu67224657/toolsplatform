$(document).ready(function () {
	...
	if (window.uno == null || window.uno.length == 0 || window.uid == null || window.uid.length == 0 || window.token == null || window.token.length == 0 ) {
		...
		var loginTip='<a href="'+url+'" class="fr" id="loginout">登录</a><span>热门评论</span>';
...
function toComment(){
	if (window.uno == null || window.uno.length == 0 || window.uid == null || window.uid.length == 0 || window.token == null || window.token.length == 0) {
		var url = window.login+'?channel='+window.channel+'&reurl='+encodeURI(opinionUri);
		location.href = url;
	} else {
...
function scroll_load(){
	...
	$("#outer-div").scroll(function(ev){
		...
		//else if(window.pnum = window.ptotal){
		//	var loadTips="<div id='loadTips'><div class='loadTips txt-c'>没有更多了</div></div>";
		//	$('#all-comment').append(loadTips);
		//}
	}); 
}
...
function inOut(obj){
	...
	if (userinout=='登录' || window.uno == null || window.uno.length == 0 || window.uid == null || window.uid.length == 0 || window.token == null || window.token.length == 0 ) {
		var url = window.login+'?channel='+window.channel+'&reurl='+encodeURIComponent(document.location.href);
		location.href = url;
		//$(obj).html('退出登录');
		$()
