$(document).ready(function () {
    var host = ".joyme." + window.marticle_host;
    window.login = 'http://passport' + host + '/auth/loginwappage'; //wap登录
    window.loginout = 'http://passport' + host + '/auth/logout'; //退出登录
    ...
    window.errorArray = {
        ...
        '-40018': '没有该条评论或评论已删除',
        '-40019': '你已经被禁言',
        '-10102': '请先<a href="clientLogin();">登录</a>之后再继续操作~'
    }
    ...
    // 判断是否登录
    if (!window.islogin) {
        var loginTip = '<a href="javascript:void(0);" class="fr" id="loginout" style="display: none;">登录</a><span>热门评论</span>';
...
function dianzan(obj) {
    if (!window.wikipage) {
        ...
        $('#agreelink_' + rid).addClass('active active-end');
        $('#hot_agreelink_' + rid).addClass('active active-end');
...
function del(obj) {
    ...
    clearInterval(timer);
    var msg = '<div><div class="wp_comment_tips">确定要删除吗?</div><div class="wp_comment_btn"><span class="wp_cancel">取消</span><span class="wp_confirm">确定</span></div></div>';
    var button = '';
    newObj = $(msg);
    newObj.find('span').click(function () {
        button = $(this).text();
        $('.wp_tc_dialog').removeClass('active');
        if (button == '确定') {
            removeComment(uniKey, domain, rid, oid);
        }
    });
    $('.wp_tc_dialog').html(newObj);
    $('.wp_tc_dialog').addClass('active');
}
    ...
    $.ajax({
        ...
        success: function (req) {
            ...
            if (resMsg.rs == '1') {
                ...
                if ((result == null || result.mainreplys.rows.length == 0) && pnum=="1") {// 没有评论
                    $('#nocomment').show();
                    $('#commentBody').hide();
                    return;
                }
                ...
                // 热门
                if (((result.hotlist == null || result.hotlist.length == 0) && result.mainreplys.page.firstPage) || result.comment_sum <= 10) {
                    $("#hot-tit, #hot-comment").hide();
                } else if (result.mainreplys.page.firstPage) {
                    if(result.comment_sum >10){
                        $("#loginTips").show();
                    }
                    var html = '';
                ...
                try{
                    _jclient.setTitle(result.comment_sum + '条评论');
                }catch(e){}
...
function getCommentListCallBack(commentObj, unikey, domain, ishot) {
    ...
    var removeHtml = '';
    if (window.uno != null && window.uid != null && window.uno == reply.user.uno && window.uid == reply.user.uid && window.wikipage == false && window.islogin) {
    ...
    var html = "<div id='" + hotid + "cont_cmt_list_" + reply.reply.rid + "' class='module-list' " +
        ...
        "<cite class='user-headImg'><img src='" + reply.user.icon + "?imageView2/0/w/42' /></cite>" +
   ...
   // $(".alert-comment").hide();
    $.ajax({
        ...
        data: {
            ...
            uid: window.uid,
            token: window.token,
            logindomain: window.logindomain
        },
        dataType: "jsonp",
        ...
        success: function (req) {
            ...
            if (resMsg.rs == '-1') {
                postSucces();
                var msg = '请先保存您的内容<br /><a href="clientLogin();">登录</a>之后再回来~';
                tips(msg);
                return;
            } else if (resMsg.rs == '1') {
                $("#textarea_body").blur();
                ...
                if (oid == 0) {// 主楼评论
                    ...
                    $('#commentBody').show();
                    var removeHtml = '';
                    if (window.uno != null && window.uid != null && window.uno == result.user.uno && window.uid == result.user.uid && window.islogin) {
                    ...
                    var commentHtml = "<div id='cont_cmt_list_" + result.reply.rid + "' class='module-list'><div class='module-infobox'>" +
                        "<cite class='user-headImg'><img src='" + result.user.icon + "?imageView2/0/w/42' /></cite>" +
                ...
                if (undefined != errorArray[resMsg.rs]) {
                    if(resMsg.rs=="-40017"){
                        $(".info-beyond span").html('内容含有敏感词：'+resMsg.result[0]);
                        $(".info-beyond").show();
...
function agreeComment(unikey, domain, rid) {
    ...
    $.ajax({
        ...
        success: function (req) {
            ...
            var resMsg = req[0];
            if (resMsg.rs == '-1') {
                var msg = '请先<a href="clientLogin();">登录</a>之后再继续操作~';
                tips(msg);
                return;
            } else if (resMsg.rs == '1') {
                ...
                if (numObj.length == 0 || objStr == null || objStr.length == 0) {
                    ...
                    if($('#hot_agreelink_' + rid).length>0){
                        num = parseInt($('#hot_agreelink_' + rid).html());
                    }
                } else {
                    var num = numObj.html();
                    num = parseInt(num);
        ...
        error: function () {
            var msg = '<a href="clientLogin();">登录</a>之后再回来~';
            tips(msg);
        }
    });
}
...
function removeComment(unikey, domain, rid, oid) {
    ...
    $.ajax({
        ...
        type: "post",
        async: false,
        data: {unikey: unikey, domain: domain, rid: rid,uno: window.uno,appkey: "default",uid: window.uid,token: window.token,logindomain: window.logindomain},
        dataType: "jsonp",
...
function scroll_load() {
    ...
    $("#outer-div").scroll(function (ev) {
        ...
        if (sTop >= sNum && window.pnum <= window.ptotal && !isload) {
            isload = true;
...
function inOut(obj) {
    ...
    if (!window.islogin) {
        ...
        clearInterval(timer);
        var msg1 = '<div><div class="wp_comment_tips">确定要退出吗?</div><div class="wp_comment_btn"><span class="wp_cancel">取消</span><span class="wp_confirm">确定</span></div></div>';
        var button = '';
        newObj = $(msg);
