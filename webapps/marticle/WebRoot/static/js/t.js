window.TouchSliderBox=(function(){
	var stack=[],
		...
		fix=function(){
			...
			SliderBox.prototype=new TouchScroll({
				id:null,
				width:7,
				opacity:0.7,
				color:'#555',
				minLength:10,
				ondrag:function(flag,distance){
					if(flag===0){
						var index,offset=parseInt(this.element.style.left),
			...
			SliderBox.prototype.slideTo=function(index){
				...
				if(index<0){
					index=0;
				}else if(index>this.imgNum-_num){
					index=this.imgNum-_num;
				}
			...
			SliderBox.prototype.prev=function(){this.slideTo(this.index-1);}

			SliderBox.prototype.next=function(){this.slideTo(this.index+1);}
		...
		if(typeof window.TouchScroll=='undefined'){
			...
			s.src="http://www.appgame.com/source/js/touchScroll.js";
		...
		return function(box,t){
			if(!t)t='sliderbox';
			if(!ready){
				stack.push(function(){this[t]=new SliderBox(box);});
			}else this[t]=new SliderBox(box);
		}
})();

window.TouchSliderBoxNav=(function(){
	var stack=[],
		ready=false,
		SliderBox,
		fix=function(){
		SliderBox=function(box){
			...
			try{
				var mobileul=document.getElementById('mobileul'),
					ul=this.container.getElementsByTagName('ul')[0],
					lis=ul.getElementsByTagName('li');
					this.index=0;
					this.imgNum=lis.length;
