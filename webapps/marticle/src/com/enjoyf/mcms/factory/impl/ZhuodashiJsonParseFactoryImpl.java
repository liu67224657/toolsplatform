...
import com.enjoyf.mcms.bean.json.*;
...
public class ZhuodashiJsonParseFactoryImpl extends AbstractParseFactoryImpl implements IHtmlParseFactory {
	...
	protected String generatorJson(Connection conn, JoymeSpec spec, List specList, String channel) throws Exception {
		...
		object.put("rs", 1);
		List<JoymePointArchiveZhuoDaShiJsonBean> jsonPointList = new ArrayList<JoymePointArchiveZhuoDaShiJsonBean>();
		for (Object listBean : specList) {
			PointArchiveListBean bean = (PointArchiveListBean) listBean;
			jsonPointList.addAll(makeJsonPointArchiveList(bean.getArchiveList(), channel));
		}
		object.put("list", jsonPointList);
	...
	private List<JoymePointArchiveZhuoDaShiJsonBean> makeJsonPointArchiveList(List<JoymePointArchive> archiveList, String channel) {
		List<JoymePointArchiveZhuoDaShiJsonBean> returnList = new ArrayList<JoymePointArchiveZhuoDaShiJsonBean>();
		...
		for (JoymePointArchive archive : archiveList) {
			...
			JoymePointArchiveZhuoDaShiJsonBean jsonBean = new JoymePointArchiveZhuoDaShiJsonBean();
			jsonBean.setTitle(archive.getTitle());
			jsonBean.setDate(archive.getArchivePublishTime());
			jsonBean.setUrl("http://marticle.joyme.com/marticle/" + archive.getHtmlPath() + "/" + archive.getHtmlFile());
