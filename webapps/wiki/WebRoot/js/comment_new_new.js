    ...
    $.ajax({
                ...
                success : function(req) {
                    ...
                    if (resMsg.rs == '0') {
                        ...
                        var replyNum = '<a href="' + morelink + '">' + result.replyNum + '<span>条评论</span></a>'
                        $('#reply_num').html(replyNum);
                        if (result.replyList == null || result.replyList.length == 0) {
                            if (result.page != null && result.page.maxPage > 1) {
                                $('#comment_list_area').html('<p style="height:240px; line-height:160px; text-align:center">当前页的评论已经被删除~</p><div class="more-comment"><a href="' + morelink + '">更多评论&gt;&gt;</a></div>');
                                $('.comment-sort').show();
                            } else {
    ...
    $.ajax({
                ...
                success : function(req) {
                    ...
                    if (resMsg.rs == '0') {
                        ...
                        $('#children_reply_list_' + rid).find('p').remove();
                        var moreDom = $('#children_reply_list_' + rid).find('div.more-comment');
                        ...
                        var num = parseInt($('#togglechildrenreply_area_' + rid).attr('data-replynum'));
                        $('#togglechildrenreply_area_' + rid).attr('data-replynum', (num + 1));
...
function removeComment(rid, rootid) {
    if (window.uno == null || window.uno.length == 0) {
        var loginHref = 'http://www.joyme.com/loginpage?reurl=' + window.location.href;
        window.location.href = loginHref;
    }
...
function getCommentListCallBack(commentObj) {
    ...
    var reCommentListHtml = '<div id="children_reply_list_' + comment.reply.replyId + '">';
    if (hasRe) {
        ...
        if (commentObj.reReplys.page != null && commentObj.reReplys.page.maxPage > 1) {
            reCommentListHtml += '<p style="text-align:center">当前页的评论已经被删除~</p><div class="more-comment"><a href="' + morelink + '">更多回复&gt;&gt;</a></div>';
        }
    }
    ...
    if (window.uno != '' && window.uno == comment.reply.replyUno) {
        removeHtml = '<a name="remove_comment" href="javascript:void(0);" data-rootid="' + comment.reply.rootId + '" data-rid="' + comment.reply.replyId + '" >删除</a>'
    }

    var toogleReHtml = '<a name="togglechildrenreply_area" href="javascript:void(0);" id="togglechildrenreply_area_' + comment.reply.replyId + '" data-replynum="' + comment.reply.replyNum + '">回复' + (comment.reply.replyNum > 0 ? ('(' + comment.reply.replyNum + ')') : '') + '</a>';
    ...
    var postReCommentHtml = '<div id="post_recomment_area_' + comment.reply.replyId + '" class="discuss_reply">' +
            ...
            ' <a class="submitbtn fr" name="submit_recomment"  data-rid="' + comment.reply.replyId + '" name="childreply_submit"><span name="span_pinglun">评 论</span></a>' +
            '</div>' +
            '</div>' +
            '</div>' +
            '</div>';
    ...
    var html = '<div id="cont_cmt_list_' + comment.reply.replyId + '" class="area blogopinion clearfix ">' +
            ...
            '<div class="discuss_bdfoot">' + comment.reply.publistDate + '&nbsp;<a href="javascript:void(0);" id="agreelink_' + comment.reply.replyId + '" data-commentid="' + comment.reply.replyId + '" class="dianzan"></a><span id="agreenum_' + comment.reply.replyId + '"><a href="javascript:void(0);" name="agree_num" data-commentid="' + comment.reply.replyId + '">' + ((comment.reply.agreeNum == null || comment.reply.agreeNum == 0) ? '' : ('(' + comment.reply.agreeNum + ')')) + '</a></span>' + removeHtml + toogleReHtml + '</div>' +
...
function getReCommentHtml(reCommentObj) {
    ...
    if (window.uno != '' && window.uno == reCommentObj.reply.replyUno) {
        removeHtml = '<a name="remove_comment" href="javascript:void(0);" data-rid="' + reCommentObj.reply.replyId + '" data-rootid="' + reCommentObj.reply.rootId + '">删除</a>'
    }
    ...
    var reCommentHtml = '<div style="" id="cont_recmt_list_' + reCommentObj.reply.replyId + '" class="conmenttx clearfix">' +
            ...
            '<span id="agreenum_' + reCommentObj.reply.replyId + '"><a href="javascript:void(0);" name="agree_num" data-commentid="' + reCommentObj.reply.replyId + '">' + ((reCommentObj.reply.agreeNum == null || reCommentObj.reply.agreeNum == 0) ? '' : ('(' + reCommentObj.reply.agreeNum + ')')) + '</a></span>' +
            removeHtml +
