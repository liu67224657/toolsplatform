...
<script language="javascript">
    ...
    <table>
        <form action="${ctx}/wiki/ac/createtemplateAction.do" name="" method="post" id="form" name="form" enctype="multipart/form-data">
            ...
            <td><input id="templateName" type="text" name="templateName" size="100" value="${template.templateName}"/>
        ...
        <tr>
            ...
            <td><select name="wiki" id="wiki">
                <c:forEach items="${wikiMap}" var="entry">
                    <option <c:if test="${template.wiki== entry}">selected</c:if>  value="<c:out value="${entry}"/>"><c:out  value="${entry}"/></option>
                </c:forEach>
            </select></td>
        </tr>
        ...
        <tr>
            ...
            <td><select name="channel" id="channel">
                <c:forEach items="${channelMap}" var="entry">
                    <option  <c:if test="${template.channel== entry.key}">selected</c:if> value="<c:out value="${entry.key}"/>"><c:out value="${entry.key}"/></option>
                </c:forEach>
            </select></td>
        </tr>
        ...
        <tr>
            ...
            <td><select name="isIndex" id="isIndex">
                <option <c:if test="${template.isIndex==0}">selected</c:if> value="0">两者</option>
                <option <c:if test="${template.isIndex==1}">selected</c:if> value="1">首页</option>
                <option <c:if test="${template.isIndex==2}">selected</c:if> value="2">子页</option>
                <option <c:if test="${template.isIndex==3}">selected</c:if> value="3">工具页</option>
            </select></td>
        </tr>
        <tr>
            <td></td>
        ...
        <tr>
            <td></td>
            <td>上传CSS</td>
            <td><input type="file" name="css" id="css" />
                </br>
                <span id="error_css" style="color:red"></span></td>
        </tr>
        <tr>
            <td></td>
            <td>模板内容</td>
            <td><textarea name="templateContext" id="templateContext" style="width:600px;height:400px">${template.templateContext}</textarea>
                </br>
