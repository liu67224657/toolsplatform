...
public class DedeAddonarticleService {
     ...
     */
    public DedeAddonarticle queryDedeAddonarticlebyId(Connection conn, int aid) throws JoymeDBException, JoymeServiceException {
        boolean isCloseConn = (conn != null) ? false : true;
        try {
            if (conn == null)
                conn = subDao.getConnection();
            return subDao.queryDedeAddonarticlebyId(conn, aid);
