...
public class SitemapFetcherService {
    ...
    private static RedisManager redisManager = PropertiesContainer.getInstance().getRedisManager();
    ...
    public static String getPath(String host, String path) {
        ...
        return PropertiesContainer.getInstance().getSitemapPath() + filePath;
    }
    ...
    public void genSitemap(Sitemap sm, String host) throws Exception {
        ...
        genXmlFile(sm.getSitemapUrl(), sm.getMappingUrl(), sm.getDomainKey());
    }

    public static void genXmlFile(String sitemapUrl, String mappingUrl, String domainKey) throws Exception {
        ...
        if (!StringUtil.isEmpty(html)) {
            ...
            //对抓取article的源sitemap进行解析
            genDocument(document, sitemapUrl, mappingUrl, domainKey);
    ...
    private static void genDocument(Document document, String sitemapUrl, String mappingUrl, String domainKey) throws Exception {
        if (document == null) {
            return;
        }
        //如果是索引文件，递归抓取
        ...
        if (!CollectionUtil.isEmpty(sitemapEles)) {
            for (Element ele : sitemapEles) {
                ...
                if (!StringUtil.isEmpty(xmlUrl) && xmlUrl.endsWith("/sitemap.xml")) {
                    genXmlFile(sitemapUrl, xmlUrl, domainKey);
                }
            }
        }
        //取出url
        ...
        if (!CollectionUtil.isEmpty(urlEles)) {
            ...
            for (Element ele : urlEles) {
                ...
                //m站抓取的是pc站的sitemap，所以需要改下链接
                if (domainKey.equals("m")) {
                    loc = loc.replace("/article/", "/article/wap/");
                }

                //取出sitemap的源链接地址
                Map<String, String> map = WebCacheUtil.genDesRule(loc);

                //www 如果article匹配不到，需要再匹配一次，因为有些规则配置/article/pc
                if (CollectionUtil.isEmpty(map) && domainKey.equals("www")) {
                    map = WebCacheUtil.genDesRule(loc.replace("/article/", "/article/pc/"));
                }

                if (!CollectionUtil.isEmpty(map)) {
                    String srcRule = map.get("srcRule");
                    String desRule = map.get("desRule");
                    loc = WebCacheUtil.genUrlByRuleurl(loc, desRule, srcRule, url.getHost());
    ...
    public static String buildSitemapUrl(String domainKey, String contextPath) {
        ...
        if (!StringUtil.isEmpty(domainKey)) {
            sitemapUrl += PropertiesContainer.getInstance().getHttpUrl() + domainKey + "." + PropertiesContainer.getInstance().getJoymeDomain();
        }
    ...
    public void pushBaiduUrls() {
        ...
        for (String host : keySet) {
            ...
            String postUrl = PropertiesContainer.getInstance().getBaiduSitemapPushApi().replace("{domainKey}", key);
