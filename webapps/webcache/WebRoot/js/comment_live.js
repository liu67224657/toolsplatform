...
var apiHost = 'http://api.joyme.' + hostname.substring(hostname.lastIndexOf('.') + 1, hostname.length) + '/';
var pid = getCookie('jmuc_pid');
var picHtml = '<div class="dialog comment_pic_d"> '+
              '<div class="dialog_con"> '+
              '<div class="close"> '+
              '<span class="close_btn"></span> '+
              '</div> '+
              '<div class="edit_pic_con"> '+
              '<div class="edit_pic_up"> '+
              '<div class="up_block upready" id="commentContainer"> '+
              '<a class="" id="commentPickfiles" href="#">+</a> '+
              '</div> '+
              '</div> '+
              '<div class="edit_pic_btn" id="comment_pic_btn"> '+
              '添加 '+
              '</div> '+
              '<div class="edit_pic_tip"> '+
              '最多添加4张图片 '+
              '</div> '+
              '</div> '+
              '</div> '+
              '</div> ';
    $('#wrapper').append(picHtml);
var bs = {
    versions: function () {
...
var commentPicNum=0;
var commentPicArray="";
var smallerSuffix = '?imageView2/1/w/300/h/300';
var waterSuffix = '?imageView2/1/w/1500%7CimageMogr/auto-orient%7Cwatermark/1/image/aHR0cDovL2pveW1lcGljLmpveW1lLmNvbS9hcnRpY2xlL3VwbG9hZHMvMTYwODE5LzgwLTE2MFE5MUZaMzQzOC5wbmc=/dissolve/70/gravity/SouthEast/ws/0.13 ';
...
if (uniKey.length > 0) {
    if (bs.versions.android || bs.versions.iPhone || bs.versions.iPad) {
        ...
        $(document).ready(function () {
            ...
            //拉取评论列表
            getMCommentList(uniKey, domain, jsonParam, 1, 10, (uid == null || uid == undefined) ? 0 : uid);
            ...
            //上传图片
            var picHtml='<span class="add_pic" id="add_pic" onclick="showMUploadWin();"></span>';
            $('.cmnt-bar').append(picHtml);
            showMUploadWin();
            //取消 清空框里面的内容
            ...
            $('.fabu').on('touchstart', function(){
                ...
                var oid = $('#input_oid').val();
                var parentId = $('#input_pid').val();
                var text = $('.new-area').val();

                text = Trim(text, 'g');

                if(parentId != oid){
                	var nick=$('#reply_sub_'+parentId).attr('data-nick');
                	text=text.replace('@'+nick+':','');
                }
                if (getStrlen(text) <= 0) {
                    $('.cmnt-beyond').html('<span>评论内容不能为空</span>');
                    $('.cmnt-beyond').show();
                    setTimeout(function(){$('.cmnt-beyond').hide()}, 2000);
                    return;
                }
                ...
                postMComment(uniKey, domain, body, oid, parentId);
                restartMDialog();
            });
...
function reCommentSubmit(dom) {
    ...
    var parentId = $(dom).attr('data-pid');
    if (parentId == null || parentId.length <= 0) {
        parentId = 0;
    }
    postSubComment(uniKey, domain, body, oid, parentId, $(dom));
}
...
function parentMask(dom) {
    ...
    submitReComment.attr("data-pid", parentId);
...
function commentSubmit() {
    ...
    //text = ToDBC(text);
    if(commentPicArray !=""){
        commentPicArray = commentPicArray.substr(0,commentPicArray.length-1);
    }
    var body = {
        text: text,
        pic: commentPicArray
                ...
                if (result.mainreplys == null || result.mainreplys.rows == null || result.mainreplys.rows.length == 0) {
                    ...
                    if (result.mainreplys.page != null && result.mainreplys.page.maxPage > 1) {
                        var pageHtml = '<div class="joyme-paging-box">' +
                            ...
                            '<div class="page">' +
                            '<ul>';
                         if (result.mainreplys.page.curPage != 1  && result.mainreplys.page.maxPage>5){
                             pageHtml += '<li><a href="javascript:void(0);" onclick="getCommentListPage(1, 10)">首页</a></li>';
                         }
                        if (result.mainreplys.page.curPage <= 3) {
                            if (result.mainreplys.page.maxPage <= 3) {
                                ...
                                for (var cp = 1; cp <= (result.mainreplys.page.maxPage>5?5:result.mainreplys.page.maxPage); cp++) {
                                    if (cp == result.mainreplys.page.curPage) {
                                        pageHtml += '<li class="thisclass">' + cp + '</li>';
                                    } else {
                                        pageHtml += '<li><a href="javascript:void(0);" onclick="getCommentListPage(' + cp + ', 10)">' + cp + '</a></li>'
                            ...
                            if (result.mainreplys.page.curPage < result.mainreplys.page.maxPage - 2) {
                                ...
                                for (var cp = (result.mainreplys.page.curPage - 2); cp <= (result.mainreplys.page.curPage + 2); cp++) {
                                    if (cp == result.mainreplys.page.curPage) {
                                        pageHtml += '<li class="thisclass">' + cp + '</li>';
                                    } else {
                                        pageHtml += '<li><a href="javascript:void(0);" onclick="getCommentListPage(' + cp + ', 10)">' + cp + '</a></li>'
                            	...
                            	if(result.mainreplys.page.maxPage > 4){
                                    for (var cp = (result.mainreplys.page.maxPage - 4); cp <= result.mainreplys.page.maxPage; cp++) {
                                        if (cp == result.mainreplys.page.curPage) {
                                            pageHtml += '<li class="thisclass">' + cp + '</li>';
                                        } else {
                                            pageHtml += '<li><a href="javascript:void(0);" onclick="getCommentListPage(' + cp + ', 10)">' + cp + '</a></li>'
                                    ...
                                    for (var cp = 1; cp <= result.mainreplys.page.maxPage; cp++) {
                                        if (cp == result.mainreplys.page.curPage) {
                                            ...
                                            pageHtml += '<li><a href="javascript:void(0);" onclick="getCommentListPage(' + cp + ', 10)">' + cp + '</a></li><a>末页</a>'
                                        }
                                    }
                            	}
                        ...
                        if (result.mainreplys.page.curPage != result.mainreplys.page.maxPage && result.mainreplys.page.maxPage>5){
                            pageHtml += '<li><a href="javascript:void(0);" onclick="getCommentListPage(' + result.mainreplys.page.maxPage + ', 10)">末页</a></li>';
                        }
                        pageHtml +='</ul>' +
                        '</div>' +
                        '</div>' +
...
function getMCommentList(unikey, domain, jsonparam, cp, psize, uid) {
    $.ajax({
        ...
        success: function (req) {
            ...
            if(result.mainreplys.page != null && result.mainreplys.page.maxPage > 1){
                moreHtml = '<div class="more-load"><a href="javascript:void(0);" onclick="getMCommentListMore('+(result.mainreplys.page.curPage + 1)+', 10)">查看更多...</a></div>';
            }
            ...
            if(result.user != null){
                ...
                userHtml += '<cite class="user"></cite><cite class="quit"><a href="javascript:loginDiv()" >登录</a></cite>';
                //userHtml += '<cite class="user"></cite><cite class="quit"><a href="'+passportHost+'auth/loginpage">登录</a></cite>';
            }	
    ...
    $.ajax({
        ...
        success: function (req) {
            ...
            if(result.mainreplys.page != null && result.mainreplys.page.maxPage > result.mainreplys.page.curPage){
                moreHtml = '<a href="javascript:void(0);" onclick="getMCommentListMore('+(result.mainreplys.page.curPage + 1)+', 10)">查看更多...</a>';
            }
...
//M 评论框评论
function postMComment(unikey, domain, body, oid, parentId) {
    var jsonParam = {
            title: title,
            pic: "",
            description: "",
    ...
    body.pic=commentPicArray;
    $.ajax({
        url: apiHost + "jsoncomment/reply/post",
        type: "post",
        data: {unikey: unikey, domain: domain, body: JSON.stringify(body), oid: oid, pid: parentId,uno:uno},
        dataType: "jsonp",
        ...
        success: function (req) {
            ...
            if (resMsg.rs == '0') {
                ...
                $('.cmnt-beyond').html('<span>参数不能为空~</span>');
                $('.cmnt-beyond').show();
                setTimeout(function(){$('.cmnt-beyond').hide()}, 2000);
                return;
            } else if (resMsg.rs == '-10102') {
                $('.cmnt-beyond').html('<span>token验证失败~</span>');
                $('.cmnt-beyond').show();
                setTimeout(function(){$('.cmnt-beyond').hide()}, 2000);
                return;
                ...
                $('.cmnt-beyond').html('<span>parentId参数错误~</span>');
                $('.cmnt-beyond').show();
                setTimeout(function(){$('.cmnt-beyond').hide()}, 2000);
                return;
...
//PC 评论框 评论
function postComment(unikey, domain, body, oid, parentId) {
    $.ajax({
        url: apiHost + "jsoncomment/reply/post",
        type: "post",
        data: {unikey: unikey, domain: domain, body: JSON.stringify(body), oid: oid, pid: parentId,uno:uno},
        ...
        success: function (req) {
            ...
            if (resMsg.rs == '0') {
                ...
                $('#reply_error').html('参数不能为空~');
                return;
            } else if (resMsg.rs == '-10102') {
                $('#reply_error').html('token验证失败~');
                return;
            } else if (resMsg.rs == '-100') {
                ...
                if (oid == 0) {
                    ...
                    $('#togglechildrenreply_area_' + oid).html('回复('+ (parseInt(numStr) + 1)+')');
                }
                var picurl = result.reply.body.pic;
                var picurlArray = null;
                var picurlHtml='<div class="picurlDiv">';
                if(picurl !=null && picurl !=undefined && picurl !="" ){
                    picurlArray = picurl.split(";");
                    for(var i=0;i<picurlArray.length;i++){
                        picurlHtml += '<span><img class="preview" src="'+picurlArray[i]+ smallerSuffix +'" data-image="' + picurlArray[i] + waterSuffix +'" /></span>';
                    }
                }
                picurlHtml += "</div>";
    ...
    commentPicNum = 0;
    commentPicArray = "";
    $('.add_comment_pic').html('');
    $('.up_block.uping').remove();
    $('.up_block.upend').remove();
}

//PC 楼中楼回复
function postSubComment(unikey, domain, body, oid, parentId, reCommentDom) {
    $.ajax({
        url: apiHost + "jsoncomment/reply/post",
        type: "post",
        data: {unikey: unikey, domain: domain, body: JSON.stringify(body), oid: oid, pid: parentId, uno:uno},
        ...
        success: function (req) {
            ...
            if (resMsg.rs == '0') {
                ...
                $('#post_callback_msg_' + oid).remove();
                reCommentDom.before('<span style="color: #f00; padding-top: 10px;margin-top:10px;padding-left: 10px;" id="post_callback_msg_' + oid + '">parentId参数错误~</span>');
                return;
            } else if (resMsg.rs == '-40010') {
                $('#post_callback_msg_' + oid).remove();
...
function initPostMask() {
    ...
    if (uno == null || uno.length == 0 || uid == null || uid.length == 0 || token == null || token.length == 0 || sign == null || sign.length == 0 || login == null || login.length == 0 || pid == null || pid.length == 0 || login == 'client') {
        ...
        '<div class="wrapper_unlogin" style="text-align:center; margin:-65px 0 65px">您需要<a id="login_link" href="#" onclick="loginDiv();">登录</a>后才能评论</div>';
    } else {
    ...
    var html = '<div id="post_reply" class="blog_comment">' +
       ...
       '<span class="fr" id="reply_num"></span>' +
        ...
        '<cite class="add_comment_pic" ></cite>'+
        ...
        '</div>';
    $('#comment_area_live').after(html);
    $('#comment_area_live').remove();
    //$('.add_comment_pic').html('');
    addCommentClick();
    initToken();
}
...
function initToken() {
    $.ajax({
        ...
        data: {profileid: pid},
        dataType: "jsonp",
        jsonpCallback: "inittokencallback",
        success: function (req) {
            var resMsg = req[0];
            uploadToken = resMsg.uploadtoken;
            commentUpload(resMsg.uploadtoken);
        }
    });
    ...
    $.ajax({
        ...
        success: function (req) {
            ...
            if (resMsg.rs == '0') {
            	...
            	var spanHtml = '条评论';
                var numHtml = $('#reply_num').text();
                var numStrH = numHtml.replace(spanHtml, '');
                var num =parseInt(numStrH);
                 if (oid == 0) {
                	 var n = $('#togglechildrenreply_area_' + rid).attr('data-replynum');
                     num=parseInt(numStrH) - parseInt(n)-1;
                     $('#cont_cmt_list_'+rid).remove()
                 } else {
                	 var numStr = $('#togglechildrenreply_area_' + oid).attr('data-replynum');
                	 num=parseInt(numStrH)-1;
                     $('#togglechildrenreply_area_' + oid).attr('data-replynum', parseInt(numStr) - 1);
...
function getCommentListCallBack(commentObj) {
    ...
    rePageHtml += '</div>';
    var picurl = reply.reply.body.pic;
    var picurlArray = null;
    var picurlHtml='<div class="picurlDiv">';
    if(picurl !=null && picurl !=undefined && picurl !="" ){
        picurlArray = picurl.split(";");
...
function referenceReply(referenceInfo) {
    $('.con_tit').children('span').eq(0).addClass('active').siblings().removeClass('active');
    $('.con_cont').children('div').eq(0).addClass('active').siblings().removeClass('active');
    var appendInfo = '<div class="replyText" ><span>' + referenceInfo + '</span></div><br/>';//contenteditable="false"
    $('#zhibo_edit_text').append(appendInfo);
    $('#zhibo_edit_text').focus();
}
//PC 楼中楼的 分页 html
...
function getMCommentListCallback(commentObj) {
    ...
    if(commentObj.subreplys != null && commentObj.subreplys.page != null && commentObj.subreplys.page.maxPage > 1){
        moreReHtml += '<a id="more_re_mcoment_'+commentObj.reply.reply.rid+'" href="javascript:void(0);" class="level-more" onclick="getReMCommentListPage('+(commentObj.subreplys.page.curPage+1)+', 5, '+commentObj.reply.reply.rid+')">查看更多...</a>';
    }
    var picurl = commentObj.reply.reply.body.pic;
    var picurlArray = null;
    var picurlHtml='<div class="picurlDiv">';
    if(picurl !=null && picurl !=undefined && picurl !="" ){
        picurlArray = picurl.split(";");
    ...
    var html = '<div id="cmnt-box-'+commentObj.reply.reply.rid+'" class="cmnt-box">' +
        ...
        '</a>' +
        '<font class="fn-left">' +
        '<a href="javascript:void(0);" class="userName">' + commentObj.reply.user.name + '</a>' +
        '</font>' +
        '</span>' +
        ...
        '<dd class="border-b">' +
        '<span>' +
        atHtml + commentObj.reply.reply.body.text +picurlHtml +
        '</span>' +
        ...
        '<div class="discuss_bdfoot"><em class="replyRef" onclick="referenceReply(\'' + commentObj.reply.user.name + ':' + commentObj.reply.reply.body.text + '\')">引用</em>&nbsp;&nbsp;' + '<code class="cmnt-time fn-clear">'+commentObj.reply.reply.post_date+'</code>'
...
function toJump(oid, parentId) {
    ...
    $('#input_oid').val(oid);
    $('#input_pid').val(parentId);
    $('#add_pic').css('display','none');
    if($('.cmnt-addpic').length>0){
        $('.cmnt-addpic').remove();
    }
    $('.pl-box').show();
    $('.mask').show();
...
function getReMCommentHtml(reCommentObj){
    ...
    var html = '<p id="reply_sub_'+reCommentObj.reply.rid+'" name="m_sub_reply" onclick="toJump('+reCommentObj.reply.oid+','+reCommentObj.reply.rid+')" data-nick="' + reCommentObj.user.name + '" data-oid="' + reCommentObj.reply.oid + '" data-pid="' + reCommentObj.reply.rid +'"><b>'+reCommentObj.user.name;
    if(puserHtml==''){
    	html+=':';
    }
...
function createCommentPicBox(){
    if(commentPicNum>=4){
        getAlert('最多只能上传4张图片');
        return;
    }
    var commentPicHtml='<div class="up_block uping" id="comment_upload_pic_'+commentPicNum+'">'+
        '<div class="up_block_con"><img id="comment_pic_'+commentPicNum+'" src="" alt=""></div>'+
        '<cite class="up_block_remove" data-index="'+commentPicNum+'" onclick="removeLiveCommentPic(this)"></cite>'+
        '<div class="up_block_line" id="comment_up_block_line_'+commentPicNum+'">'+
        '</div>';
    $('#commentContainer').before(commentPicHtml);

}
//删除图片
function removeLiveCommentPic(dom){
    var index=$(dom).attr('data-index');
    var removePicurl = $('#comment_pic_'+index).attr('src');
    $('#comment_upload_pic_'+index).remove();
    commentPicNum--;
    commentPicArray = commentPicArray.replace(removePicurl + ";","");
    if(commentPicNum==0){
        $('.add_comment_pic').html('');
    }else{
        $('.add_comment_pic').html('<em>'+commentPicNum+'<em>');
    }

}

//保存图片
function saveCommentPic(){
    $('.dialog.comment_pic_d').css('display','none');
    if(commentPicNum<=0){
        getAlert('未上传图片')
        return;
    }
    //commentPicArray=new Array(commentPicNum);
    for(var i=0;i<commentPicNum; i++){
        //commentPicArray[i]=$('#comment_pic_'+i).attr('src');
        var imgsrc = $('#comment_pic_'+i).attr('src');
        if (commentPicArray==""){
            commentPicArray +=imgsrc + ";" ;
        }else if(commentPicArray.indexOf(imgsrc)==-1){
            commentPicArray +=$('#comment_pic_' + i).attr('src') + ";";
        }
    }
    getAlert('保存成功');
...
function addCommentClick(){
    $('#comment_pic_btn').on('click',function(){
        saveCommentPic();
    });
    var pageLoad={
        init:function(){
        ...
        showdialog:function(){
            ...
            $('.close_btn').on('click',function(){
                ...
                if(ancestordom.hasClass('comment_pic_d')){
                    commentPicNum=0;
                    commentPicArray="";
                    $('.add_comment_pic').html('');
...
//图片传进度条
function commentUploadPicProcessLine(uploadRate){
    $('#comment_up_block_line_'+commentPicNum).html('<span style="width:'+uploadRate+'%">'+uploadRate+'%</span>');
}
function showCommentUploadPic(sourceLink){
    $('#comment_pic_'+commentPicNum).attr('src',sourceLink);
    $('#comment_upload_pic_'+commentPicNum).removeClass('uping').addClass('upend');
    commentPicNum++;
    $('.add_comment_pic').html('<em>'+commentPicNum + '</em>');
}
function commentUpload(uploadToken){

    var QiniuPic = new QiniuJsSDK();
    var picUploader = QiniuPic.uploader({
        runtimes: 'html5,flash,html4',    //上传模式,依次退化
        browse_button: 'commentPickfiles',       //上传选择的点选按钮，**必需**
        uptoken: uploadToken,
        unique_names: false ,
        save_key: false,
        ...
        init: {
            ...
            'UploadProgress': function (up, file) {
                ...
                commentUploadPicProcessLine(uploadRate);
            ...
            'FileUploaded': function (up, file, info) {
                try {
                    ...
                    showCommentUploadPic(sourceLink);
                } catch (ex) {
                    alert(ex);
                }
            },
...
function showMUploadWin() {
    $('#add_pic').css("display","block");
    if($('.cmnt-addpic').length>0){
        $('.cmnt-addpic').remove();
    }else{
        var mpichtml = '<div class="cmnt-addpic" id="mpicDiv"><a class="" href="#" id="mCommentPickfiles">+</a></div>';
        $('.cmnt-success').after(mpichtml);
        mInitToken();
    }

}
var mCommentPicNum=0;
function createMCommentPicBox(){
    if(mCommentPicNum>=4){
        getAlert('最多只能上传4张图片');
        return;
    }
    var mCommentPicHtml='<div class="up_block uping" id="mcomment_upload_pic_'+mCommentPicNum+'">'+
        '<div class="up_block_con"><img id="mcomment_pic_'+mCommentPicNum+'" src="" alt=""></div>'+
        '<cite class="up_block_remove" data-index="'+mCommentPicNum+'" onclick="mRemoveCommentPic(this)"></cite>'+
        '<div class="up_block_line" id="mcomment_up_block_line_'+mCommentPicNum+'">'+
        '</div>';
    $('#mpicDiv').before(mCommentPicHtml);

}
//删除图片
function mRemoveCommentPic(dom){
    var index=$(dom).attr('data-index');
    var removePicurl = $('#mcomment_pic_'+index).attr('src');
    $('#mcomment_upload_pic_'+index).remove();
    mCommentPicNum--;
    commentPicArray = commentPicArray.replace(removePicurl + ";","");
    if(mCommentPicNum==0){
        $('.add_pic').html('');
    }else{
        $('.add_pic').html('<em>'+mCommentPicNum+'<em>');
    }

}
//图片传进度条
function mCommentUploadPicProcessLine(uploadRate){
    $('#mcomment_up_block_line_'+mCommentPicNum).html('<span style="width:'+uploadRate+'%">'+uploadRate+'%</span>');
}
function mShowCommentUploadPic(sourceLink){
    $('#mcomment_pic_'+mCommentPicNum).attr('src',sourceLink);
    if(mCommentPicNum+1<=4){
        saveMcommentPic();
        $('#mcomment_upload_pic_'+mCommentPicNum).removeClass('uping').addClass('upend');
        mCommentPicNum++;
        $('.add_pic').html('<em>'+mCommentPicNum + '</em>');
    }
}
function mInitToken() {
    $.ajax({
        ...
        data: {profileid: pid},
        dataType: "jsonp",
        jsonpCallback: "minittokencallback",
        success: function (req) {
            var resMsg = req[0];
                mCommentUpload(resMsg.uploadtoken);
        }
    });

}
//m端保存图片
function saveMcommentPic(){
        if (commentPicArray==""){
            commentPicArray +=$('#mcomment_pic_'+mCommentPicNum).attr('src')+ ";";
        }else {
            commentPicArray +=$('#mcomment_pic_' + mCommentPicNum).attr('src') + ";" ;
        }
}
function mCommentUpload(mUploadtoken){

    var QiniuPic = new QiniuJsSDK();
    var picUploader = QiniuPic.uploader({
        runtimes: 'html5,flash,html4',    //上传模式,依次退化
        browse_button: 'mCommentPickfiles',       //上传选择的点选按钮，**必需**
        uptoken: mUploadtoken,
        unique_names: false ,
        save_key: false,
        domain: 'http://joymepic.joyme.com/',
        container: 'mpicDiv',           //上传区域DOM ID，默认是browser_button的父元素，
        max_file_size: '8mb',           //最大文件体积限制
        flash_swf_url: 'http://lib.joyme.com/static/third/qiniuupload/plupload/Moxie.swf',  //引入flash,相对路径
        max_retries: 3,                   //上传失败最大重试次数
        dragdrop: true,                   //开启可拖曳上传
        ...
        init: {
            ...
            'BeforeUpload': function (up, file) {
                ...
                createMCommentPicBox();
            },
            'UploadProgress': function (up, file) {
                var chunk_size = plupload.parseSize(this.getOption('chunk_size'));
                var uploadRate=Math.ceil(file.loaded*100/file.size);
                mCommentUploadPicProcessLine(uploadRate);
            ...
            'FileUploaded': function (up, file, info) {
                try {
                    ...
                    var res = JSON.parse(info);
                    var sourceLink = domain +res.key; //获取上传成功后的文件的Url
                    mShowCommentUploadPic(sourceLink);
                } catch (ex) {
                    alert(ex);
                }
            },
...
function restartMDialog() {
    for(var i = 0 ; i<mCommentPicNum ; i++){
        $('#mcomment_upload_pic_'+i).remove();
    }
    $('.add_pic').html('');
    $('.cmnt-addpic').remove();
    mCommentPicNum = 0;
