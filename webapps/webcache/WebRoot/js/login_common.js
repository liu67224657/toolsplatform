...
(function () {
    var mod = {
        init: function () {
            var html = '<script src="http://static.joyme.com/tools/luosimao/api.js"></script><div class="success-btn">注册成功</div><div class="loginbarBox"><div class="login-box login-bg"><h1>账号登录</h1><span class="close-icon">关闭</span><div class="login-con">' +
                    '<div class="login-text"><label><span>手机号：</span><input type="text" id="l_phone"></label><i class="text-error on" id="l_phone_i"></i></div>' +
                    ...
                    '<i class="text-error on" id="r_password_i"></i></div>' +
                    ...
                    '<input type="hidden" id="luotest_response_mima" name="luotest_response_mima" value="" >' +
                    '<div class="sure-btn w-200" id="forgot">确定</div></div><a href="javascript:;" class="login login-mask mask-link fn-r">已有账号？去登录</a ></div><div class="mask-box"></div>' +
                    '<div class="captcha-box">'+
                    '<div class="l-captcha" data-site-key="533a7e232fb9134c30928ceebad087ef" data-width="250" data-callback="getResponse">'+
                    '</div>'+
                    '</div>'+
                    '</div>';
...
//            });
            ...
            $("#login").click(function () {
                ...
                if (!loginlock) {
                    ...
                    $.ajax({
                        url: apiHost + "platform/auth/login",
                        dataType: "jsonp",
                        jsonp: "callback",
        ...
        registerInit: function () {
            ...
            $("#register").click(function () {
                ...
                if (!valid1 || !valid2 || !valid3  || !valid5) {
                    return;
                }
                var agree = $("#agree").is(":checked");
                if (!agree) {
                ...
                if (!registerlock) {
                    ...
                    registerlock = true;
                    $.ajax({
                        url:apiHost + "platform/auth/register",
                        dataType: "jsonp",
                        jsonp: "callback",
                        jsonpCallback: "platformauthcallback",
                        data: {
            ...
            $("#forgot").click(function () {
                ...
                if (!forgotlock) {
                    ...
                    forgotlock = true;
                    $.ajax({
                        url: apiHost + "platform/auth/recover/password",
                        dataType: "jsonp",
                        jsonp: "callback",
                        jsonpCallback: "platformauthcallback",
                        data: {
            ...
            $("#" + id + "_sendmobile").click(function () {
               // alert("aaaaaaaaaaaaaa"+mod.captcha)
                ...
                if (!bool) {
                    ...
                    var url = apiHost + "platform/auth/mobile/sendcode";
                    if (id == 'f') {
                        url = apiHost + "platform/auth/existsmobile/sendcode"
                    }
                    $.ajax({
                        url: url,
                        dataType: "jsonp",
