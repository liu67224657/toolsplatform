...
var ucHost = 'http://uc.joyme.' + hostname.substring(hostname.lastIndexOf('.') + 1, hostname.length) + '/';
...
//加载其他页面的评论
var _uniKey = $('#_uniKey').val();
var _domain = $('#_domain').val();
if (typeof(_uniKey) != "undefined") {
    uniKey = _uniKey;
}
    ...
    if (bs.versions.android || bs.versions.iPhone || bs.versions.iPad) {
        ...
        $(document).ready(function () {
            ...
            $('.fabu').on('touchstart', function () {
                if (uno == null || uid == null) {
                    $('.cmnt-beyond').html('<span>请注意保存您的内容，登录之后再回来~</span>');
                    $('.cmnt-beyond').show();
                    setTimeout(function () {
                        $('.cmnt-beyond').hide()
                    }, 2000);
...
function reMCommentSubmit(dom) {
    ...
    if (getStrlen(text) > 140) {
        $('#reply_error').html('评论内容长度不能超过140字符!');
        return false;
    }
    ...
    postMSubComment(uniKey, domain, body, oid, pid, $(dom));
}
            ...
            if (resMsg.rs == '1') {
                ...
                if (result.mainreplys == null || result.mainreplys.rows == null || result.mainreplys.rows.length == 0) {
                    ...
                    if (result.mainreplys.page != null && result.mainreplys.page.maxPage > 1) {
                        ...
                        // pageHtml += '<li><span class="pageinfo">共<strong>' + result.mainreplys.page.maxPage + '</strong>页<strong>' + result.mainreplys.page.totalRows + '</strong>条</span></li>' +
    ...
    $.ajax({
        ...
        success: function (req) {
            ...
            if (resMsg.rs == '0') {
                ...
                $('#cmnt-list').html('');
                getMCommentListMore(1, 5)

//                var oid = result.reply.oid;
//                if(oid == null || oid == 0){
//                    var html = '<div id="cmnt-box" class="cmnt-box">' +
//                        '<dl>' +
//                        '<dt>' +
//                        '<span class="fn-clear">' +
...
//                        '</a>' +
//                        '<font class="fn-left">' +
//                        '<a href="javascript:void(0);" class="userName">' + result.user.name + '</a><code>' + result.reply.post_date + '</code>' +
//                        '</font>' +
//                        '</span>' +
//                        '<cite class="cmnt-reply fn-right" onclick="toJump(' + result.reply.rid + ', '+result.reply.rid+')">回复</cite>' +
//                        '<cite class="zan-num fn-right" data-commentid="' + result.reply.rid + '"onclick="agreeClick(this)">'+ ((result.reply.agree_sum == null || result.reply.agree_sum == 0) ? '' : ('(' + result.reply.agree_sum + ')')) +'</cite>'+
//                        '</dt>' +
//                        '<dd>' +
//                        '<span class="border-b">' +
//                        result.reply.body.text +
//                        '</span>' +
//                        '<div class="cmnt-level">' +
//                        '<div class="level-con" id="re_mcomment_list_'+result.reply.rid+'">' +
//                        '</div>' +
//                        '</div>' +
//                        '</dd>' +
//                        '</dl>' +
//                        '</div>';
//                    if($('.cmnt-list div').length == 0){
//                        $('.cmnt-list').append(html);
//                    }else{
//                        $('.cmnt-list div:first').before(html);
//                    }
//                    
//                }else{
//                    var puserHtml = '';
//                    if (result.puser != null && result.puser.name != null) {
//                        puserHtml = '<i>回复</i><b>'+result.puser.name+'：</b>';
//                    }
//                    var html = '<p><b>'+result.user.name+'：</b>'+puserHtml + result.reply.body.text +'</p>';
//                    if($('#re_mcomment_list_'+oid+' p').length == 0){
//                        $('#re_mcomment_list_'+oid).append(html);
//                    }else{
//                        $('#re_mcomment_list_'+oid+' p:first').before(html);
//                    }
//                }
                $('#input_oid').val(0);
                $('#input_pid').val(0);
...
//M 楼中楼回复
function postMSubComment(unikey, domain, body, oid, pid, reCommentDom) {
    $.ajax({
        url: apiHost + "jsoncomment/reply/post",
        type: "post",
        data: {unikey: unikey, domain: domain, body: JSON.stringify(body), oid: oid, pid: pid},
        ...
        success: function (req) {
            ...
            if (resMsg.rs == '0') {
                ...
                var commentHtml = '<div id="cont_cmt_list_' + result.reply.rid + '" class="area blogopinion clearfix ">' +
                    '<dl>' +
                    '<dt class="personface">' +
                    '<a title="' + result.user.name + '" target="_blank" name="atLink" href="' + ucHost + 'usercenter/page?pid=' + result.user.pid + '">' +
                    '<img width="58" height="58" class="user" src="' + result.user.icon + '"></a>' +
                    '</dt>' +
                    '<dd class="textcon discuss_building">' +
                    '<a title="' + result.user.name + '" target="_blank" class="author" name="atLink" href="' + ucHost + 'usercenter/page?pid=' + result.user.pid + '">' + result.user.name + '</a><code>' + result.reply.floor_num + '楼</code>' +
                    ...
                    '</span><a onclick="removeClick(this)" name="remove_comment" href="javascript:void(0);" data-oid="0" data-rid="' + result.reply.rid + '">删除</a>' +
                    '<a name="togglechildrenreply_area" href="javascript:void(0);" onclick="toggle(this)" id="togglechildrenreply_area_' + result.reply.rid + '" data-replynum="0">回复</a>' +
                    '</div>' +
...
function getCommentListCallBack(commentObj) {
    ...
    if (uno != null && uid != null && token != null && sign != null && login != null && login != 'client' && uno == reply.user.uno && uid == reply.user.uid) {
        removeHtml = '<a onclick="removeClick(this)" name="remove_comment" href="javascript:void(0);" data-oid="' + reply.reply.oid + '" data-rid="' + reply.reply.rid + '" >删除</a>'
    }
    ...
    var html = '<div id="cont_cmt_list_' + reply.reply.rid + '" class="area blogopinion clearfix ">' +
        '<dl>' +
        '<dt class="personface">' +
        '<a title="' + reply.user.name + '" name="atLink" href="' + ucHost + 'usercenter/page?pid=' + reply.user.pid + '" target="_blank">' +
        '<img width="58" height="58" class="user" src="' + reply.user.icon + '">' +
        '</a>' +
        '</dt>' +
        '<dd class="textcon discuss_building">' +
        '<a title="' + reply.user.name + '" class="author" name="atLink" href="' + ucHost + 'usercenter/page?pid=' + reply.user.pid + '" target="_blank">' + reply.user.name + '</a><code>' + reply.reply.floor_num + '楼</code>' +
...
function getMCommentListCallback(commentObj) {
    ...
    var html = '<div id="cmnt-box-' + commentObj.reply.reply.rid + '" class="cmnt-box">' +
        ...
        '</a>' +
        '<font class="fn-left">' +
        '<a href="javascript:void(0);" class="userName">' + commentObj.reply.user.name + '</a><code>' + commentObj.reply.reply.post_date + '</code><code class="floor">' + commentObj.reply.reply.floor_num + '楼</code>' +
        '</font>' +
        '</span>' +
        '<cite class="cmnt-reply fn-right" onclick="toJump(' + commentObj.reply.reply.rid + ', ' + commentObj.reply.reply.rid + ')">回复</cite>' +
        '<cite class="zan-num fn-right" data-commentid="' + commentObj.reply.reply.rid + '"onclick="agreeClick(this)">' + ((commentObj.reply.reply.agree_sum == null || commentObj.reply.reply.agree_sum == 0) ? '' : ('(' + commentObj.reply.reply.agree_sum + ')')) + '</cite>' +
        '</dt>' +
        '<dd>' +
        '<span class="border-b">' +
...
function getReMCommentHtml(reCommentObj) {
    ...
    var html = '<p><b>' + reCommentObj.user.name + '：</b>' + puserHtml + reCommentObj.reply.body.text + '</p>';
    return html;
}
...
function getReCommentHtml(reCommentObj) {
    ...
    if (uno != null && uid != null && token != null && sign != null && login != null && login != 'client' && uno == reCommentObj.user.uno && uid == reCommentObj.user.uid) {
        removeHtml = '<a onclick="removeClick(this)" name="remove_comment" href="javascript:void(0);" data-rid="' + reCommentObj.reply.rid + '" data-oid="' + reCommentObj.reply.oid + '">删除</a>'
    }
    ...
    var reCommentHtml = '<div style="" id="cont_recmt_list_' + reCommentObj.reply.rid + '" class="conmenttx clearfix">' +
        ...
        '<a href="' + ucHost + 'usercenter/page?pid=' + reCommentObj.user.pid + '" target="_blank" title="' + reCommentObj.user.name + '" name="atLink" class="cont_cl_left">' +
        '<img width="33px" height="33px" src="' + reCommentObj.user.icon + '">' +
        '</a>' +
        '</div>' +
        '</div>' +
        '<div class="conmentcon">' +
        '<a title="' + reCommentObj.user.name + '" name="atLink" href="' + ucHost + 'usercenter/page?pid=' + reCommentObj.user.pid + '" target="_blank">' + reCommentObj.user.name + '</a>' +
