$(document).ready(function() {
    ...
    $(document).click(function(event) {
        ...
        var textAreas = $('textarea[id^=textarea_recomment_body_]:show');
...
function initPostMask() {
    if (window.uno == null || window.uno.length == 0) {
        $('#textarea_body').after('<div class="wrapper_unlogin">您需要<a id="login_link" href="http://www.joyme.com/loginpage?reurl=' + window.location.href + '">登录</a>后才能评论</div>');
...
function getCommentList(cid, sort) {
    $.post("/article/comment/list.do", {cid:cid,sort:sort}, function(req) {
        var resMsg = req;
        if (resMsg.rs == '0') {
            return;
        } else {
...
function getHotCommentList(contentid) {
    $.post("/article/comment/hotlist.do", {cid:contentid}, function(req) {
        var resMsg = req;
        if (resMsg.rs == '0') {
            return;
        } else {
            var result = resMsg.result.replyList;
            if (result == null || result.length == 0) {
                $('#comment_list_area').html('<p style="height:240px; line-height:160px; text-align:center">目前没有评论，欢迎你发表评论</p>')
            } else {
                var html = '';
                for (var i = 0; i < result.length; i++) {
                    html += getCommentListCallBack(result[i]);
                }
                var moreLink = '<div class="more-comment"><a href="' + morelink + '">更多评论&gt;&gt;</a></div>';
                html += moreLink;
                $('#comment_list_area').html(html);
...
function postComment() {
    ...
    var body = $('#textarea_body').val().trim();
...
function postreComment(rid, pid, body, reCommentDom) {
    ...
    $('#hidden_recomment_cid').val(contentid);
...
function agreeComment(rid) {
    ...
    $.post("/article/comment/agree.do", {rid:rid}, function(req) {
        var resMsg = req;
        if (resMsg.rs == '-1') {
            var loginHref = 'http://www.joyme.com/loginpage?reurl=' + window.location.href;
            window.location.href = loginHref;
