...
public class JoymeappBaseJDBCDAOImpl implements BaseJDBCDAO {
    /* 当前读取的数据源 */
    public static String currentReadDataSource = BaseJDBCDAO.joymeappReadJndiName;
    ...
    private void initMainReadDS(String jndiName) throws JoymeDBException {
        if (mainReadDs == null) {
            jndiName = (jndiName == null) ? BaseJDBCDAO.joymeappReadJndiName : jndiName;
    ...
    public void switchToMainDs() {
        if (!isMainReadDataSource) {// 如果不是主读取源，切换回来
            try {
                ...
                currentReadDataSource = BaseJDBCDAO.joymeappReadJndiName;
    ...
    public Connection getConnection() throws JoymeDBException {
        ...
        try {
            ...
            if (isMainReadDataSource) {
                this.initMainReadDS(joymeappReadJndiName);
                ds = mainReadDs;
                isEntered = true;
            }

            if(hasBackupDataSource && !isEntered){
                this.initBackupReadDS(joymeappReadJndiName);
                ds = backupReadDs;
            }
...
//            LogService.infoBaseDaoLog("Password:"+ pds.getPassword(), true);
            ...
            if (hasBackupDataSource) { // 如果有备份，走备份
                try {
                    ...
                    String dsName = currentReadDataSource.equals(BaseJDBCDAO.joymeappReadJndiName) ? joymeappReadJndiName : joymeappReadJndiName;
