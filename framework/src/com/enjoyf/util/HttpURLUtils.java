...
public final class HttpURLUtils {
    ...
    public static byte[] getBytes(String reqUrl, Map<String, String> parameters) throws IOException {
        HttpURLConnection conn = sendGet(reqUrl);
        return getBytes(conn);
    }

    private static byte[] getBytes(HttpURLConnection urlConn) throws IOException {
        InputStream in = null;
    ...
    public static byte[] getByte(String picPath, boolean isLocal) throws IOException {
        byte[] picData = null;
        if (isLocal) {
            ...
            picData = HttpURLUtils.getBytes(picPath, null);
        }
        return picData;
    }


    public static String getValueByRequest(HttpServletRequest request,String key){
        return StringUtil.isEmpty(request.getParameter(key)) ?
                request.getHeader(key) : request.getParameter(key);
    }
}
