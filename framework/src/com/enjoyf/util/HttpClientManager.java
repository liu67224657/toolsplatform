...
public class HttpClientManager {
    ...
    public HttpResult multPartURL(String url, HttpParameter[] params)
            throws Exception {
        MultipartPostMethod methodpost = new MultipartPostMethod(url);
        for (HttpParameter param : params) {
            methodpost.addParameter(param.getName(), param.getValue());
        }
        HttpMethodParams param = methodpost.getParams();
        param.setContentCharset("UTF-8");
    ...
    private HttpResult httpRequest(HttpMethod method, HttpParameter[] headerParams) {
        ...
        try {
            ...
            String resultStr = readInputStream(method.getResponseBodyAsStream());
    ...
    public byte[] getByte(String picPath, boolean isLocal) throws IOException {
        byte[] picData = null;
        if (isLocal) {
